...
```
# 假设是3分类问题，真实类别是类别1
hard_label = [0, 1, 0]  # one-hot编码

# 模型被训练成：
# 对正确类别：输出概率应该 = 1.0（100%确信）
# 对错误类别：输出概率应该 = 0.0（0%可能）

# 使用label smoothing，ε = 0.1
smooth_label = [0.05, 0.9, 0.05]

# 模型被训练成：
# 对正确类别：输出概率 ≈ 0.9（90%确信）
# 对错误类别：输出概率 ≈ 0.05（5%可能）
```
...
$$
y_i^{LS} = \begin{cases}
1 - \epsilon + \frac{\epsilon}{K} & \text{if } i = k \\
...
### Data mix-up
**理解：**
通过**线性插值**混合两个训练样本及其标签，生成新的虚拟训练样本，从而提高模型的泛化能力和鲁棒性
...
x_mixed = λ * x1 + (1-λ) * x2  # 混合图片
y_mixed = λ * y1 + (1-λ) * y2  # 混合标签

# x_mixed = 60%的猫图 + 40%的狗图
# y_mixed = [0.6, 0.4] (这张图片有是60%猫, 40%是狗)

model.train(x_mixed, y_mixed)
```

**计算过程：**
**取出两张图片的预测和turth label**
![[Pasted image 20250929155616.png]]
![[Pasted image 20250929155623.png]]

**采样**
$$λ \sim Beta(α, α)\;\; \alpha = 
\begin{cases} 
0.2 & \text{λ更可能接近0或1} \\ 
0.5 & \text{中等倾向} \\ 
1 & \text{λ在[0,1]均匀分布} 
\end{cases} $$
- 从Beta分布采样$\lambda$，通常α = 0.2 或 1.0

**计算x与y的预测值**
![[Pasted image 20250929162732.png]]
...
- $x_i, x_j$: 两个随机采样的训练样本
- $y_i, y_j$: 对应的标签（one-hot编码）
- $λ$: 混合系数，λ ∈ [0, 1]

**优化器更新**
$$\text{minimize}\; CE(\tilde{y},p(\tilde{x}))$$
...
$$
\lambda = 1 - \frac{r_w \times r_h}{W \times H}
$$
- $r_x, r_y, r_w, r_h$: 裁剪框的位置和大小
- $λ$: 混合比例，由裁剪区域面积决定
- $W, H$: 图像的宽度和高度

**裁剪框生成：**
![[Pasted image 20250929162925.png]]
$$\begin{aligned}
& r_w = W \sqrt{1 - \lambda}\\
& r_h = H \sqrt{1 - \lambda}\\
& r_x \sim \text{Uniform}(0, W)\\
& r_y \sim \text{Uniform}(0, H)

\end{aligned}$$
$$Box = [r_x,r_y,r_w,r_h]$$

**裁剪**
![[Pasted image 20250929163619.png]]
$M \in{\{0,1\}}^{W\times H}$  by filling 1 within **B** and 0 otherwise

**预测基底图片的概率**
![[Pasted image 20250929162835.png]]

$$
\tilde{x} = \mathbb{M} \odot x_i + (1 - \mathbb{M}) \odot x_j
$$
- $\mathbb{M}$: 二值掩码矩阵（1表示保留$x_i$，0表示使用$x_j$）
- $⊙$: 逐元素相乘（Hadamard积）
...
**作用：**
- 图像清晰，特征完整，同时学习两个类别的局部特征
- 更好的泛化，模型不能只依赖中心区域
- 鲁棒性提升

| 特性       | Mixup  | CutMix | Cutout |
| -------- | ------ | ------ | ------ |
| **混合方式** | 像素级加权  | 区域替换   | 区域遮挡   |
| **标签处理** | 软标签混合  | 软标签混合  | 不变     |
| **视觉效果** | 模糊/半透明 | 清晰拼接   | 黑色遮挡   |
| **定位能力** | 差      | 强      | 中      |
| **计算开销** | 小      | 小      | 极小     |
...
**公式:**
BN usually use before Activations
...
**理解：**
在深度神经网络训练过程中，由于前一层参数的更新，导致当前层输入数据的分布不断发生变化的现象

**举例：**
```
# 网络结构
x → Layer1(W1) → a1 → Layer2(W2) → a2 → Layer3(W3) → output
...
# Iteration 2:
W1更新为W1_new → a1的分布变成: N(0.5, 1.5)  ← 分布改变了！
Layer2之前学到的W2现在不适用了，需要重新适应新分布

# Iteration 3:
W1再次更新 → a1的分布又变成: N(-0.3, 0.8)  ← 又变了！
Layer2又要重新适应...
```
