...
==当一个问题难以通过精确的数学公式直接求解时，我们可以通过大量的随机抽样和模拟实验，来估算出一个近似的答案==

#### 步骤
1. **建立概率模型 (Define the Model)**： 为你要解决的问题建立一个数学模型。明确其中的**随机变量**（即不确定的因素）和它们的**概率分布**
    - _例如：在项目管理中，每个任务的完成时间不是固定的，可能服从一个正态分布或三角分布
    
2. **进行大量随机抽样 (Perform Random Sampling)**： 从已定义的概率分布中，为每个随机变量生成大量的随机数值。每一次抽样都构成一个完整的“场景”或“试验”
    - _例如：模拟 10,000 次项目，每次都为所有任务随机抽取一个完成时间

3. **对每次抽样进行确定性计算 (Run Deterministic Calculation)**： 在每个生成的随机场景下，运行你的模型，计算出一个确定的结果
    - _例如：在每一次模拟中，根据抽样得到的任务时间，计算出整个项目的总完成时间

4. **汇总和统计分析 (Aggregate and Analyze Results)**： 将所有模拟得到的结果汇总起来，进行统计分析（如计算平均值、方差、中位数，或绘制直方图）
    - _例如：分析那 10,000 个项目的总完成时间，你可以得到“项目有 90% 的概率在 50 天内完成”这样的结论
    
### Monte Carlo Tree Search-MCTS
| 特性       | Alpha-Beta 搜索                                     | 蒙特卡洛树搜索 (MCTS)                                               |
| -------- | ------------------------------------------------- | ------------------------------------------------------------ |
| **搜索方式** | 深度优先，穷尽式地探索一个固定的深度。                               | **选择性地**、非对称地构建搜索树，重点探索最有希望的区域。                              |
| **评估方式** | 依赖一个**手工设计的、复杂的启发式评估函数** `EVAL(state)` 来给非终局局面打分。 | **不需要复杂的评估函数**。通过大量的**随机模拟（“Playouts”或“Rollouts”）**来评估局面的优劣。 |
| **树的构建** | 树的结构是固定的，由游戏规则决定。                                 | 树是**逐步增长**的，算法会把更多的计算资源投入到更有价值的分支上。                          |
| **适用性**  | 适用于分支因子不太大，且容易设计评估函数的游戏（如国际象棋）。                   | 适用于分支因子巨大，难以评估的游戏（如围棋）。                                      |
#### 步骤
a. 选择 (Selection)

- **目标**：从根节点（当前真实的游戏局面）开始，沿着树向下走，直到找到一个**可以被扩展**的节点。
- **策略**：在每一步选择子节点时，算法会遵循一个策略，这个策略需要在**“探索” (Exploration)** 和 **“利用” (Exploitation)** 之间取得平衡。
    - **利用**：选择当前看起来胜率最高的子节点（已经被证明是好棋）。
    - **探索**：选择那些被访问次数较少的子节点（也许是潜力股，值得一试）。
- **实现**：最常用的策略是 **UCT (Upper Confidence bounds applied to Trees)** 公式。简单来说，它会给每个子节点计算一个分数，这个分数综合了该节点的**当前胜率**和它的**被探索次数**。算法会选择分数最高的那个子节点。

b. 扩展 (Expansion)

- **目标**：当“选择”步骤到达一个叶子节点（一个还未被完全探索的节点）时，就在这个节点上扩展搜索树。
- **策略**：从这个叶子节点所有可能的合法走法中，随机选择**一个或多个**未被加入到树中的走法，并将它们作为新的子节点添加到树中。

c. 模拟 (Simulation) - _这是“蒙特卡洛”的核心体现_

- **目标**：快速评估“扩展”步骤中新加入的那个节点的价值。
- **策略**：从这个新节点开始，**进行一次完全随机的模拟对局**，也称为 **"Playout"** 或 **"Rollout"**。也就是说，让对弈双方都随机地乱下，直到游戏分出胜负。
- **特点**：这个过程非常快，因为它不涉及任何复杂的思考，纯粹是随机选择。虽然单次随机模拟的结果很不可靠，但成千上万次模拟的统计结果却能惊人地准确。

d. 反向传播 (Backpropagation)

- **目标**：将“模拟”步骤中得到的对局结果（赢或输）更新回搜索树中。
- **策略**：从模拟的起始节点开始，沿着“选择”步骤走过的路径，一路返回到根节点。路径上的每一个节点，其统计数据都会被更新：
    - **访问次数 (visits) +1**
    - 如果模拟结果是赢，则 **胜利次数 (wins) +1**
e. 决策

- 当分配的思考时间用完后（例如，运行了 100,000 次迭代），算法会查看根节点的所有直接子节点（即当前所有可能的走法）。它会选择那个**被访问次数最多**的子节点作为最终的决策。这通常被认为是最稳健的选择，因为它意味着算法在这个分支上投入了最多的计算资源，并且经过大量的模拟验证，认为它综合来看是最好的
