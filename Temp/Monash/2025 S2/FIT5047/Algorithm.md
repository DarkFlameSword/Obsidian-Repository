...
```
==步骤:==
1. **初始化**：创建一个`Open Set`和一个``。将起点放入开放列表。
2. **循环**：只要开放列表不为空，就执行以下操作： 
    a. 从开放列表中找到启发函数值 `h(n)` 最小的节点，称之为当前节点 `current`。 
    b. 将 `current` 从开放列表移到关闭列表。 
    c. 检查目标：如果 `current` 就是目标节点，则搜索成功，返回路径。 
    d. 扩展节点：获取 `current` 的所有邻居节点。 
    e. 对于每个邻居节点： * 如果它已经在关闭列表中，则忽略它。 * 如果它不在开放列表中，则计算它的 `h(n)` 值，设置其父节点为 `current`，然后将它加入开放列表
#### A
