...
$$\phi_1 = (((A ∨ (B → C)) ∧ (¬C ∨ D)) → ((A → D) ∨ (C ↔ E)))$$
**Converting steps:**
1. eliminate $\rightarrow \text{and} \leftrightarrow$: $\left( \lnot \left[ A \lor (\lnot B \lor C) \right] \land (\lnot C \lor D) \right) \lor \left( (\lnot A \lor D) \lor \left[ (C \land E) \lor (\lnot C \land \lnot E) \right] \right)$
2. CNF: $\left[ (\lnot A \land B \lor \lnot C) \land (\lnot C \lor D) \right] \lor \left[ (\lnot A \lor D) \lor \ (C \land E) \lor (\lnot C \land \lnot E) \right]$
		$\left[ (\lnot A \lor \lnot C) \land (B \lor \lnot C) \land (\lnot C \lor D) \right] \lor \left[ (\lnot A \lor D) \lor \ (C \land E) \lor (\lnot C \land \lnot E) \right]$
